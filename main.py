...
def split_dataset(
    data: pandas.DataFrame, test_size=0.2, random_state=7
) -> typing.Tuple:
    """
...
def run_experiment(experiment_id, n_splits=5):
    ...
    for model_name, model in models.items():
        ...
        with mlflow.start_run(
            experiment_id=experiment_id, run_name=run_id
        ):
            ...
            print(
                f"Accuracy: {cv_results.mean():.3f} ({cv_results.std():.3f})"
            )
            ...
            for fold_idx, kflod_result in enumerate(cv_results):
                mlflow.log_metric(
                    key="crossval", value=kflod_result, step=fold_idx
                )
...
def get_best_model(experiment_id, metric):
    ...
    model_uri = (
        f"runs:/{best_run.info.run_id}/{best_run.data.params['model_name']}"
    )
...
def register_best_model(experiment_id, metric, registered_model_name):
    """
    Register the best model in the experiment as a new model in the
...
def move_model_to_production_from_run_name(
        experiment_id, run_name, model_name
):
    """
    ...
    if found_run.info.run_id is None:
        raise Exception(f"Run {run_name} not found.")

    model_uri = (
        f"runs:/{found_run.info.run_id}/{found_run.data.params['model_name']}"
    )
    model = None
    try:
